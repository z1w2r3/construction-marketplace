...
基于参考文档的格式模板,从当前知识库智能提取数据并生成排版一致、内容不同的新文档。
...
在整个执行过程中,遵循以下原则:
1. **格式优先**: 严格保持参考文档的排版格式
2. **内容真实**: 所有数据必须来自知识库或用户提供,绝不编造
3. **多样化**: 每次生成的内容都不同,降低查重率
4. **可追溯**: 所有信息标注来源文档
5. **用户确认**: 关键信息需用户确认
...
if [ ! -f "$1" ]; then
  echo "❌ 错误: 参考文档不存在: $1"
  echo "请提供有效的 Word 文档路径(.docx)"
  exit 1
fi

# 验证文件格式
if [[ ! "$1" =~ \.docx$ ]]; then
  echo "⚠️  警告: 建议使用 .docx 格式的参考文档"
  echo "当前文件: $1"
  echo "是否继续? (y/n)"
fi
```
...
如果参数为空:
```
❌ 使用方法: /notebook-clone-format <参考文档路径>

示例:
  /notebook-clone-format ./templates/智能建造实施方案-模板.docx
  /notebook-clone-format ~/Documents/申报资料模板.docx

💡 提示: 参考文档应为标准的 Word .docx 格式
```

#### 步骤 0.2: 读取知识库配置

从 `.notebooklm/config.md` 读取知识库路径:

```markdown
如果未找到配置文件:
❌ 错误: 知识库未初始化
请先运行: /notebook-init 初始化知识库
